...
RUN_ID="${RUN_ID:-backfill-daisy_steamgen-dlinear-full-20260720}"
LOG="${LOG:-/root/daisy_steamgen_dlinear_full_${RUN_ID}.log}"
MARKER="${MARKER:-/root/daisy_steamgen_DLINEAR_FULL_OK}"
PYTHON="${PYTHON:-/root/autodl-tmp/OmniSelect/.venv/bin/python}"
exec > >(tee -a "${LOG}") 2>&1
echo "lane=daisy_steamgen-DLinear-full run_id=${RUN_ID} start=$(date -Is)"
export RUN_ID TS_DATASET=daisy_steamgen TS_MODEL=dlinear
...
for SEED in 0 1 2; do
  export SEED
  echo "===== daisy_steamgen DLinear SEED=${SEED} start=$(date -Is) ====="
  set +e
...
touch "${MARKER}"
echo "lane=daisy_steamgen-DLinear-full end=$(date -Is)"
