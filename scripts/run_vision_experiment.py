...
def main():
    ...
    for m in METHODS:
        ...
        acc = float((clf.predict(Xt) == test_lab).mean())
    ...
    _trial_dump(results, "vision", DATASET, SEED,
                {"noise_frac": NOISE_FRAC, "vis_noise": os.environ.get("VIS_NOISE", "inject"),
                 "drop": os.environ.get("DROP_CHANNEL", ""), "pool": POOL_N,
                 "budget": BUDGET_FRAC, "val_n": VAL_N,
