...
and `docs/provenance_evidence/` (hash lists + license snapshots). Raw datasets are
...
| DaISy CSTR [98-002] | KU Leuven SISTA DaISy | `data/daisy/cstr.dat` | `0ffdda8a1b962d377dc34371be105bd9dcaef7fcca40554e666841efeec6b84d` | concentration forecast | No formal license published; citation-required academic-use statement snapshotted (`daisy_usage_terms.txt`), distributed as download pointer + SHA only, file not in release |
...
| TEP | Braatz ML-ready `d00..d21(+_te)` | 44 `.dat` files, per-file list → `docs/provenance_evidence/tep_files_sha256.txt` | sorted-concat digest `f1df2998f0417cf934a4fcc998a5bb86ab1baf6c66779eea20a6ada5bf91a72f` | normal + fault classes 1–21 | license unverified, distributed as download pointer + SHA only, files not in release |
...
section). If these files are absent from your checkout, they ship in the release
archive (URL: pending. The release archive has NOT been published yet; until it is, these artifacts are not downloadable and no one-click reproduction of them is claimed).
...
- Raw datasets (`data/raw/`, `data/processed/`, `data/*` payloads), re-obtainable
...
- Run outputs (`outputs/`): per-seed `results.json` trees, embedding caches
  (40 `.npz`, 2.1 GiB, hashed in place in the POST_DATA_LOCK inventory), model files.
- The 308-file POST_AUDIT2 **experiment-artifact** backup (logs / results / docs /
  ...
  data-integrity lock).
- `experiments/POST_DATA_LOCK_inventory.json` (raw/derived/cache inventory with
  per-file SHA256) if not present in your checkout.
